...
class DataArguments:
    ...
    image_base_path: str = field(
        default="data/tongagent", metadata={"help": "Root Path to the training data."}
    )
...
def rank0_alert():
    if local_rank == 0:
        import wandb
        wandb.init(project="minicpm")
        wandb.alert(title="Training starts", text="Training starts")
...
def make_supervised_data_module(
    ...
    train_dataset = dataset_cls(
        ...
        image_base_path=data_args.image_base_path
    )
...
def train():
    ...
    if os.path.exists(training_args.output_dir):
        ...
        training_args.output_dir = f"{training_args.output_dir}_{now}"
