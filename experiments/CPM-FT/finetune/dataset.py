    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        try:
            if "image" not in self.raw_data[i]:
                ...
                image_path = os.path.join(self.image_base_path, self.raw_data[i]["image"])
                ...
                images_dict = dict()
                for img_name, img_path in self.raw_data[i]["image"].items():
            ...
            # print("set system prompt", system_prompt)
            # print("first message", conversations[0]["content"])
            ret = preprocess(
                images_dict,
                conversations,
                self.tokenizer,
            ...
            logger.error(f"data fetch error {repr(e)} for {self.raw_data[i]['id']}")
            raise e
        return ret
...
def conversation_to_ids(conversation, tokenizer, llm_type=None, new_schema=False, max_length=2048, system_prompt=None):
    """
    ...
    if llm_type == "llama3":
        ...
        input_ids, context, raw_msg = conversation_to_ids_qwen2(
            conversation, tokenizer, system_prompt=system_prompt
        )
    else:
    ...
    if len(image_start_tokens) != len(image_end_tokens):
        logger.error(f"image start token != image end tokens item {conversation[0]['content']}")
...
def conversation_to_ids_qwen2(conversation, tokenizer, system_prompt=None):
    raw_msg = ""
    chat = [] if system_prompt is None else [{"role": "system", "content": system_prompt}]
    context = []
    for idx, msg in enumerate(conversation):
        role = msg["role"]
        message = msg["content"]
...
def preprocess(
    ...
    if len(images_dict) == 1 and "<image>" in images_dict:       
        ...
        input_dict = conversation_to_ids(conversations, tokenizer, llm_type, new_schema, max_length, system_prompt)
    else:
        pattern = r'<image_\d+>'
        new_conversations = []
